...
```{r}
sector_plot<- ggplot(Uncleaned_DS_jobs, aes(x=Sector)) + 
  labs(title = "Distribution of Sector", x = "Sector", subtitle = "") + 
...
```{r}
size_plot<- ggplot(Uncleaned_DS_jobs, aes(x=Size)) + 
  labs(title = "Distribution of Size", x = "Size", subtitle = "") + 
  geom_bar(colour="darkblue", fill="lightblue") + 
  geom_text(stat='count', aes(label=..count..), vjust=-0.5,size=2.2) +
  theme_classic()+
   theme(axis.text.x = element_text(size = 9.2, angle = 45, hjust = 1))
...
Expected 2 pieces in each element of `Location`. ! 
672 values were too short. ℹ Use `too_few = "debug"` to diagnose the problem.
...
Uncleaned_DS_jobs <- Uncleaned_DS_jobs %>%
  mutate(
    Location = case_when(
      ...
      Location == "Utah" ~ str_replace(Location, "Utah", "Utah, Utah"),
      Location == "New Jersey" ~ str_replace(Location, "New Jersey", "New Jersey, New Jersey"),
      Location == "Texas" ~ str_replace(Location, "Texas", "Texas, Texas"),
      Location == "California" ~ str_replace(Location, "California", "California, California"),
