...
\<\<\<\<\<\<\< HEAD Computer Scientists:

```{r}
str_view(
  Uncleaned_DS_jobs$Job_Title, 
  regex(".*Computer\\s+Scientist.*|.*Computational\\s+Scientist.*|.*Computational.*", 
        multiline = TRUE, 
        ignore_case = TRUE))
```
...
Uncleaned_DS_jobs$Job_Title <-  str_replace_all(
  Uncleaned_DS_jobs$Job_Title,
  ".*Computer\\s+Scientist.*|.*Computational\\s+Scientist.*|.*Computational.*",
  "Computer Scientist")
```

We assigned Computer Scientist here. But, there is a problem.
...
As we can see, `str_replace_all()` is case sensitive. And we cannot put `ignore_case = TRUE` in `str_replace_all()`. So we have to apply this to upper case as well.

```{r}
Uncleaned_DS_jobs$Job_Title <-  str_replace_all(
  Uncleaned_DS_jobs$Job_Title,
  ".*COMPUTER\\s+SCIENTIST.*|.*COMPUTATIONAL\\s+SCIENTIST.*",
  "Computer Scientist")
```
...
str_view(
  Uncleaned_DS_jobs$Job_Title, 
  regex(".*Computer\\s+Scientist.*|.*Computational\\s+Scientist.*", 
        multiline = TRUE, 
        ignore_case = TRUE))
```
...
Uncleaned_DS_jobs$Job_Title <-  
  str_replace_all(
    Uncleaned_DS_jobs$Job_Title,
    ".*Analytics\\s+Manager.*|.*Data\\s+Science\\sManager.*|.*Director.*|.*Vice\\sPresident.*|.*VP.*|.*Principal.*|.*Manager.*","Data Science and Analytics Manager")
```
...
```{r}
str_view(Uncleaned_DS_jobs$Job_Title, regex("^(?!.*Data\\s+Analyst.*|.*Data\\s+Scientist.*|.*Data\\s+Science\\s+and\\s+Analytics\\s+Manager.*|.*Data\\sEngineer.*).*data.*", ignore_case = TRUE))
```

```{r}
summary(as.factor(Uncleaned_DS_jobs$Job_Title))
```
